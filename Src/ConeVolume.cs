...
{
	public partial class ConeVolume : Gtk.Window
	{
		...
		/// The cys.
		/// </summary>
		private ConeSolve cs = new ConeSolve();
		...
		/// <summary>
		/// Raises the button new cone clicked event.
		/// </summary>
		/// <param name="sender">The source of the event.</param>
		/// <param name="e">Instance containing the event data.</param>
		protected void OnBtnNewConeClicked(object sender, EventArgs e)
		{
			this.ClearData();
		}
		...
		/// </summary>
		/// <param name="sender">The source of the event.</param>
		/// <param name="e">Instance containing the evnent data.</param>
		...
		private bool ConvertDataToInches()
		{
			...
			val = this.cs.GetTheDiameterTotalInches(
			...
			val = this.cs.GetTheHeightTotalInches(
		...
		/// <c>false</c> otherwise.</returns>
		private bool ValidateCubicAreaConeData()
		{           
			bool retVal = false;
			const int Cnt = 6;
		...
		private void SolveForMetricUnitsStandardUnits()
		{
			...
			if (this.metricUnits)
			{
				retVal = this.ValidateCubicAreaConeData();
				if (!retVal)
				{
					return;
				}
